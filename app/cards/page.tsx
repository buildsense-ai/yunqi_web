...
export default function CardsPage() {
  ...
  const handleDeleteEvent = async (eventId: number) => {
    try {
      const response = await axiosClient.get(`/api/events/${eventId}?action=delete`)
  ...
  const handleUpdateEvent = async (eventId: number, data: { summary?: string; category?: string; status?: string }) => {
    try {
      const response = await axiosClient.put(`/api/events/${eventId}`, data)
  ...
  const handleDeleteImage = async (eventId: number, messageId: string) => {
    try {
      await axiosClient.delete(`/api/events/${eventId}/images/${messageId}`)
  ...
  const handleAddImage = async (eventId: number, imageData: any) => {
    try {
      const response = await axiosClient.post(`/api/events/${eventId}/images`, imageData)
  ...
  const handleDeleteDocument = async (eventId: number, messageId: string) => {
    try {
      await axiosClient.delete(`/api/events/${eventId}/documents/${messageId}`)
  ...
  const handleDeleteMessage = async (eventId: number, messageId: string) => {
    try {
      await axiosClient.delete(`/api/events/${eventId}/messages/${messageId}`)
  ...
  const handleMergeEvents = async () => {
    ...
    try {
      ...
      const response = await axiosClient.post("/api/merge-events", selectedEventIds)
